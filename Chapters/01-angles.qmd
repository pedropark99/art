...
the singular form of the word "vertices". It is a point where two lines meet and form an angle,
or, you could also say that a vertex is a single edge
point of an object.
...
### Disabling expansion when limiting the Cartesian plane

By default, ggplot2 always adds some little extra space to the
limits of the Cartesian plane. It does that, because the plot doesn't
feel (or look) too much tight with this little extra space.

If you pay close attention to the x and y axis over the previous examples,
that we used `coord_cartesian()` to limit the Cartesian plane,
you will probably notice that these axis are, in fact, using the range 9 to 11.25,
instead of using the range 9.15 to 11.15.

This is the little extra space that ggplot2 added to all sides of the
plot. But sometimes, we want to avoid that. We need to define the limits
of our image precisely, and we want to avoid any type of automatic "padding"
that ggplot2 might add to it.

In order to do that, we need to set the `expand` argument of 
`coord_cartesian()` to `FALSE`. By doing this, you can see in the example
below that no extra space is added, and the Cartesian plane is
precisely limited to the range 9.15 to 11.15, as we wanted to.
...
### The direction of the rotation {#sec-angle-direction}

But one thing that not many people remember from math class, is that
angles can also be negative. That is, an angle can also be a negative number, like an angle of
-35 degrees. But when you change the sign of the number, you change
the "direction" of the angle.

In more details, the sign of the angle (if it is a positive or
...
**in trigonometry, angles are measured in radians**. As
consequence, every function used in trigonometry (like tangent or
cosine) uses values in radians as input, instead of values in degrees.

Using radians instead of degrees to measure an angle changes nothing about
what the angle actually is, and it also changes nothing about how the
sign of the angle changes the direction of the rotation made by that
angle, as I showed at @sec-angle-direction.
...
into radians values, and vice-versa. Given that $d$ is an angle in
...
We also have an inverse function for each of one of these
three main trigonometric functions. These functions are commonly referred to
as the "arc functions".
...
As a first example, let's use a circle with 5 points of radius,
and that is centered at the origin point. If you want to
calculate the coordinates of the circumference at angles 35 degrees,
and 68 degrees for example, you would get approximately the
coordinates $(4.10, 2.87)$ and $(1.87, 4.64)$, as you can see below:
...
At @fig-circle-coordinates you can visualize what these coordinates are,
and how they relate back to the circle in the Cartesian plane:


```{r}
#| echo: false
#| warning: false
#| fig-cap: "Calculating coordinates in the circumference of a circle"
#| fig-width: 7.5
#| fig-height: 7
#| fig-dpi: 200
#| label: "fig-circle-coordinates"
...
df <- tibble(
  x = xr(5, deg2rad(c(35, 68))),
  y = yr(5, deg2rad(c(35, 68))),
)
...
main_circle <- tibble(
  x = xr(5, deg2rad(seq(0, 90, by = 1))),
  y = yr(5, deg2rad(seq(0, 90, by = 1)))
)


circle1 <- tibble(
  x = xr(1, deg2rad(seq(0, 68, by = 1))),
  y = yr(1, deg2rad(seq(0, 68, by = 1)))
)

circle2 <- tibble(
  x = xr(1.6, deg2rad(seq(0, 35, by = 1))),
  y = yr(1.6, deg2rad(seq(0, 35, by = 1)))
)


annos <- tibble(
  x = c(1.5, 3, 0.85, 2, 4.10, 2.5),
  y = c(2.8, 1.75, 1.25, 0.75, 3.1, 4.64),
  a = c(58, 28, 58, 26, 0, 0),
  c = c("black", "black", "#804016", "#102e0e", "black", "black"),
  t = c("Length 5", "Length 5", "Angle of\n68 degrees", "Angle of\n35 degrees",
        "(4.10, 2.87)", "(1.87, 4.64)")
)
...
canvas +
  ...
  geom_text(
    aes(x, y, label = t, angle = a, colour = c),
    data = annos,
    size = 14/.pt,
    family = "Inconsolata"
  ) +
...
As a second example, let's suppose you wanted to calculate the
