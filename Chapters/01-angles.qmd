...
More precisely, it is a data visualization framework with focus on
statistics, data exploration and data analysis. Because of that, the
output of ggplot2 code is more commonly referred to as a "plot".
...
But one thing that not many people remember, is that angles can also be
...
you do that, you change the "direction" of the angle.
...
Although we are used to talk about angles using the degrees measure, in
trigonometry, angles are measured using the radians measure instead.
This changes nothing about what an angle actually is, or how the signal
of the angle changes the direction of the rotation made by that angle.
The angle is just measured using a different scale.

The radians scale is completely based on the PI value, which is denoted
by the $\pi$ character. If you ever need to use the PI value in R,
...
With very little trigonometry, you can calculate the coordinates of any
point in the circumference of a circle. If you want to draw a circle,
you most certainly need to calculate the coordinates of the
circumference.
...
coordinates (e.g. if you trying to calculate the position of the very
top of the circle - the highest point in the circle, then the angle
should be $-\pi/2$, which is 90 degrees counter-clockwise), you can
...
circle <- data.frame(
  x = 3 * cos(deg2rad(angles)),
  y = 3 * sin(deg2rad(angles))
)
