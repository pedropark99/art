...
You can do this by using the function `deg2rad()` below. This function
uses the @eq-deg2rad that we introduced at Chapter 1 to convert these
measures. Notice that this function uses the PI value (i.e. the built-in
`pi` object) to make this conversion.
...
there is another pair of equations that you can use to calculate rotations,
and you already saw them in this book, which is
@eq-rotation3 and @eq-rotation4 that were presented in Chapter 1.

These two equations are useful to me, because they are
the easiest pair of equations to use when you want to rotate specifically a line,
or a stroke if you prefer to call it this way.
...
certain angle. I just think that these equations are less intuitive than
@eq-rotation3 and @eq-rotation4 for this specific case.
