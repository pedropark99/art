...
But it probably will be wrong on the logic, and will likely not
...
the angle of one of the triangle's edges. In contrast, in the second scenario, we
...
So when you do use directly @eq-rotation1 and @eq-rotation2, instead
of getting scenario 1 or scenario 2 of @fig-linerotation, you would get a visual result that would looks
something like this:
...
l <- line_length(0, 0, 2.2, 2.1)

angles <- seq(0, 360, by = 1)
circle <- data.frame(
  x = l * cos(deg2rad(angles)),
  y = l * sin(deg2rad(angles))
)



A <- deg2rad(35)
# acos( (v1x * v2x + v1y * v2y) / (sqrt(v1x^2 + v1y^2) * sqrt(v2x^2 + v2y^2)) )
current_angle <- acos( (l * 2.2 + 0 * 2.1) / (sqrt(l^2 + 0^2) * sqrt(2.2^2 + 2.1^2)) )
...
rotated_line <- line |>
  mutate(
    x = c(0, l * cos(new_angle)),
    y = c(0, l * sin(new_angle))
  )
...
ggplot() +
  geom_line(
    aes(x, y, color = "Vertical line"),
    data = data.frame(x = c(0, 0), y = c(0, l))
  ) +
  geom_line(
    aes(x, y, color = "Original line"),
    data = line
