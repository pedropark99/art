...
### `runif()` and `rnorm()` in two dimensions
...
with 100x100 area. That is, an image with 100 pixels of with and 100 pixels of height.
...
way, each element in the matrix represents a spot/pixel in your image.
...
You could also, store this sequence of random values inside a `data.frame`,
and then, you add two new columns in this `data.frame` to store the x and y coordinates
that represents each spot/pixel in your image. This is also
