...
That is why, we usually say that computers can only generate
...
I get a different answer, or a different set of random values, as result:
...
Every time I run `runif()`, R automatically and randomly selects a seed for me.
...
of number generation, we must use
...
to 40, and, because of that, you can see that I get the exact same set of random values
...
or to improve it by adding some adjustments in configuration,
...
If you want to favor a specific value in your sample (i.e. if you want
that specific value to appear more in the sample), you can use
the `prob` argument of `sample()` to assign a higher probability
to that specific value in detriment of the other values.

In essence, the `prob` argument receives a vector of probabilities
for obtaining the elements of the vector that is being sampled.
The sum of this vector of probabilities does not need to be
equal to one.

But I think you should create this vector of probabilities
with the "sum equal to one" in mind. Because it makes
easier for you to think about how much weight, or how
much value you want to give to a specific value
in detriment of the others.

In our previous example, the `pallete` vector contains
5 different colors. So the initial probability of choosing
each color is $1/5=0.2$, or, 20%.

But let's suppose
I want to use the dark blue (`#023047`) more in the plot.
If that is my case, then, I want to assign a higher probability
for this color. As an example, let's use 60% for this specific
color. You can see in the result below that the dark blue
color is much more present now:

```{r}
p_other_elements <- (1 - 0.6) / 4
p_dark_blue <- 0.6

probs <- rep(p_other_elements, 5)
probs[3] <- p_dark_blue
set.seed(60)
df$color <- sample(
  pallete, size = 100, replace = TRUE,
