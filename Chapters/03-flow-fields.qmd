...
Now, if we execute this function with the exact same configs we used at
the first example that we drawn that red curve. If you look at the example below,
you can see that this curve
is clearly going into the opposite direction compared to 
the curve we drawn in the first example.

In the snippet below, we are using the same starting position
($x = 20$ and $y = 25$), the same flow field, the same
number of steps, and same step length. If you compare
this curve to the curve.
...
merge these two directions together, to achieve the curve we want
to achieve.

The `both_directions()` function below merges these two directions
...
We can now see the end result of this function
